      ...
      <transition name="slide">
        <div v-if="guacMenu" class="sidebar-panel" id="guac-menu">
          <ul class="sidebar-panel-nav" id="clipboard-settings">
            <h3>클립보드</h3>
            <div class="content">
              <p>데스크톱에서 복사하거나 잘라낸 텍스트가 여기에 표시됩니다. 텍스트 변경 사항은 원격 클립보드에 직접 적용됩니다.</p>
          ...
          <ul class="sidebar-panel-nav" id="keyboard-settings">
            ...
            <div class="content">
              <div class="choice">
                <label><input id="ime-none" name="input-method" ng-change="closeMenu()" ng-model="menu.inputMethod" type="radio" value="none"> 없음</label>
                <p class="caption"><label for="ime-none">사용중인 입력 방법이 없습니다. 키보드 입력은 열결된 물리적 키보드에서 받아들여집니다.</label></p>
              </div>
              <div class="choice">
                <div class="figure"><label for="ime-text"><img src="../../assets/tablet-keys.svg" alt=""></label></div>
                <label><input id="ime-text" name="input-method" ng-change="closeMenu()" ng-model="menu.inputMethod" type="radio" value="text"> 텍스트 입력</label>
                <p class="caption"><label for="ime-text"> 텍스트 입력을 허용하고, 입력된 텍스트를 바탕으로 키보드 이벤트를 에뮬레이트 합니다. 이것은 물리적 키보드가 없는 휴대폰과 같은 장치에 필요합니다.</label></p>
              </div>
              <div class="choice">
                <label><input id="ime-osk" name="input-method" ng-change="closeMenu()" ng-model="menu.inputMethod" type="radio" value="osk"> 화상 키보드</label>
                <p class="caption"><label for="ime-osk"> 내장된 데스크톱 화상 키보드의 입력을 표시하고 허용합니다. 화상 키보드는 다른 방법으로는 불가능할 수 있는 키 조합을 입력할 수 있습니다. (Ctrl-Alt-Del 등)</label></p>
              </div>
            </div>
          </ul>
          <ul class="sidebar-panel-nav" id="mouse-settings">
            ...
            <div class="content">
              ...
              <div class="choice">
                  ...
                  <div class="figure">
                      <label for="absolute"><img src="../../assets/touchscreen.svg" alt="터치 스크린"></label>
                      <p class="caption"><label for="absolute">탭하여 클릭합니다. 클릭은 터치 위치에서 발생합니다.</label></p>
                  </div>
              </div>
              <!-- Touchpad -->
              <div class="choice">
                  ...
                  <div class="figure">
                      <label for="relative"><img src="../../assets/touchpad.svg" alt="터치 패드"></label>
                      <p class="caption"><label for="relative">드래그하여 마우스 포인터를 움직이고 탭하여 클릭합니다. 클릭은 마우스 포인터 위치에서 발생합니다.</label></p>
                  </div>
              </div>
            </div>
          </ul>
              ...
              <div id="zoom-settings">
                <div class="client-zoom">
                  <div class="client-zoom-editor">
                    <div ng-click="zoomOut()" class="client-zoom-out"><img src="../../assets/zoom-out.svg" alt="-"></div>
                    ...
                    <div ng-click="zoomIn()" class="client-zoom-in"><img src="../../assets/zoom-in.svg" alt="+"></div>
                  </div>
                  <div class="client-zoom-autofit">
                    <label><input ng-model="client.clientProperties.autoFit"
                                  ng-change="changeAutoFit()"
                                  ng-disabled="autoFitDisabled()" type="checkbox" id="auto-fit"> 브라우저 창에 자동으로 맞춤</label>
                  </div>
                </div>
              </div>
            </div>
...
<script>
  ...
  import encrypt from "@/components/lib/encrypt";
  import dis from "@/components/lib/display";
  import states from "@/components/lib/states";
  import clipboard from "@/components/lib/clipboard";
  import token from "@/components/lib/param.json";
  import Modal from "./Modal";

  const wsUrl = `ws://10.10.1.29:8083/`
